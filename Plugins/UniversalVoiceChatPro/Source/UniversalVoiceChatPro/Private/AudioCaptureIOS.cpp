...
OSStatus FAudioCaptureIOS::OnCaptureCallback(AudioUnitRenderActionFlags* ioActionFlags, const AudioTimeStamp* inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList* ioData)
{
	...
	status = AudioUnitRender(IOUnit,
		...
		BufferList);
	if (status != noErr) {
		UE_LOG(LogVoiceChatPro, Warning, TEXT("AudioUnitRender error %d"), status);
		return 0;
	}
...
{
	if (wasInit) {
		UE_LOG(LogVoiceChatPro, Warning, TEXT("iosOpenCaptureStream was already init"));
		return true;
	}

	
	UE_LOG(LogVoiceChatPro, Warning, TEXT("iosOpenCaptureStream %d %d"), _SampleRate, _NumChannels);
	...
	{		
		UE_LOG(LogVoiceChatPro, Warning, TEXT("error AVAudioSessionCategoryPlayAndRecord"));
		return false;
	}
	...
	if (setCategoryError != nil)
	{
		UE_LOG(LogVoiceChatPro, Warning, TEXT("error setActive"));
		return false;
	}
	...
	if (Status != noErr) {
		UE_LOG(LogVoiceChatPro, Warning, TEXT("AudioComponentInstanceNew error %d"), Status);		
		return false;
	}
	// Enable recording
	uint32 EnableIO = 1;
	Status = AudioUnitSetProperty(IOUnit,
		...
		sizeof(EnableIO));
	if (Status != noErr) {
		UE_LOG(LogVoiceChatPro, Warning, TEXT("AudioUnitSetProperty error %d"), Status);
		return false;
	}
	...
	Status = AudioUnitSetProperty(IOUnit,
		...
		sizeof(EnableIO));
	if (Status != noErr) {
		UE_LOG(LogVoiceChatPro, Warning, TEXT("AudioUnitSetProperty 1 error %d"), Status);		
		return false;
	}
	...
	if (Status != noErr) {		
		UE_LOG(LogVoiceChatPro, Warning, TEXT("AudioUnitSetProperty 2error %d"), Status);
		return false;
	}
	...
	Status = AudioUnitSetProperty(IOUnit,
		...
		sizeof(CallbackInfo));
	if (Status != noErr) {		
		UE_LOG(LogVoiceChatPro, Warning, TEXT("AudioUnitSetProperty 3 error %d"), Status);
		return false;
	}
	...
	if (Status != noErr) {		
		UE_LOG(LogVoiceChatPro, Warning, TEXT("AudioUnitInitialize error %d"), Status);
		return false;
	}
